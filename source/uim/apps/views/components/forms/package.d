...
this(DView myView, string myName, DOOPEntity myEntity) { this(myView, myName).entity(myEntity); }
` : ``)~
(withEntities ? `
...
this(DView myView, string myName, DOOPEntity[] myEntities) { this(myView, myName).entities(myEntities); }
` : ``);
}
...
auto `~name~`(DView myView, DOOPEntity myEntity) { return new D`~name~`(myView, myEntity); }
auto `~name~`(string myName, DOOPEntity myEntity) { return new D`~name~`(myName, myEntity); }
auto `~name~`(DView myView, string myName, DOOPEntity myEntity) { return new D`~name~`(myView, myName, myEntity); }
` : ``)~
(withEntities ? `
auto `~name~`(DOOPEntity[] myEntities) { return new D`~name~`(myEntities); }
auto `~name~`(DView myView, DOOPEntity[] myEntities) { return new D`~name~`(myView, myEntities); }
auto `~name~`(string myName, DOOPEntity[] myEntities) { return new D`~name~`(myName, myEntities); }
auto `~name~`(DView myView, string myName, DOOPEntity[] myEntities) { return new D`~name~`(myView, myName, myEntities); }
` : ``);
}
