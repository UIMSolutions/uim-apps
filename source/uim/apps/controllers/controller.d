...
class DAPPController : DH5AppController {
    this() { super(); }
    ...
    DAPPControllerMiddlewareOptions middleware(DAPPMiddleware middleware, STRINGAA options = null) {
    ...
    */
        return new DAPPControllerMiddlewareOptions(options);
    }

    // The middleware registered on the controller.
    protected DAPPMiddleware _middleware;
    // Get the middleware assigned to the controller.
    DAPPMiddleware middleware() {
        return _middleware; }
...
auto APPController(DAPPUIM myApp) { return new DAPPController(myApp); }
auto APPController(DAPPUIM myApp, string myName) { return new DAPPController(myApp, myName); }
auto APPController(DAPPUIM myApp, DETBBase myDatabase) { return new DAPPController(myApp, myDatabase); }
auto APPController(DAPPUIM myApp, string myName, DETBBase myDatabase) { return new DAPPController(myApp, myName, myDatabase); }
